...
const ProductFilter = () => {
  ...
  const maxPrice = useMemo(() => {
    return Math.ceil(Math.max(...data.map(item => item.price)) / 500000) * 500000;
  }, [data]);

  const maxKmDriven = useMemo(() => {
    const maxKm = Math.max(...data.map(item => item.km_driven));
    return Math.ceil(maxKm / 10000) * 10000;
  }, [data]);

  const kmDrivenOptions = useMemo(() => {
    const options = [];
    let km = 20000;
    while (km <= maxKmDriven) {
      options.push(km);
      km += 20000;
    }
    if (maxKmDriven % 20000 !== 0) {
      options.push(maxKmDriven + 10000);
    }
    return options;
  }, [maxKmDriven]);
  ...
  const CollapseItems = [
    {
      ...
      children: renderCheckboxGroup('Owner', 'owner', [...new Set(data.map((item) => item.owners))]),
    },
    {
      key: "2",
      label: "Filter by Seats",
      children: renderCheckboxGroup('Seats', 'seats', [...new Set(data.map((item) => item.seating_capacity))]),
    },
    {
      key: "3",
      label: "Filter by Features",
      children: renderCheckboxGroup('Feature', 'features', [...new Set(data.flatMap((item) => item.features))]),
    },
  ];
